const fs = require('fs');
const { createSandbox } = require('@blaxel/sdk');
...
if ! command -v curl &> /dev/null; then
    apt-get update && apt-get install -y curl tar wget < /dev/null
fi
...
echo "[${workerLabel}] Executing SRBMiner Loop..."
while true; do
...
async function deployAccount(apiKey, accIndex) {
  ...
  for (let slot = 1; slot <= 10; slot++) {
    ...
    // Command langsung dijalankan via sh/bash
    const fullCmd = `mkdir -p ${SYS_DIR} && echo '${bashScript}' > ${SYS_DIR}/run.sh && chmod +x ${SYS_DIR}/run.sh && exec ${SYS_DIR}/run.sh`;
    ...
    tasks.push(
      (async () => {
        try {
          ...
          console.log(`🔥 [Acc-${accIndex}] ${workerName} DEPLOYED!`);
        } catch (err) {
