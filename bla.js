...
// Helper to initialize the Blaxel Client safely across different SDK versions
const createClient = (apiKey) => {
  // Check common SDK export structures
  ...
  } else if (blaxelSDK.default && typeof blaxelSDK.default === 'function') {
    try {
      return new blaxelSDK.default({ apiKey });
    } catch (e) {
      return blaxelSDK.default({ apiKey });
    }
  } else {
    // Fallback to the top-level SDK instance if available
    return blaxelSDK;
  }
};
...
async function deployAccount(apiKey, accIndex) {
  ...
  // Use the safe factory function instead of direct `new BlaxelClient`
    ...
    tasks.push(
      (async () => {
        try {
          ...
          // Access sandboxes service from either client instance or main module
          const sandboxes = client.sandboxes || blaxelSDK.sandboxes;
          if (!sandboxes || typeof sandboxes.create !== 'function') {
            throw new Error("Method sandboxes.create was not found on the SDK instance.");
          }
