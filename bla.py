...
# Import module internal Blaxel SDK versi 0.4.x
try:
    from blaxel.core.sandbox import Sandbox
except ImportError:
    try:
        from blaxel.core.sandboxes import Sandbox
    except ImportError:
        print("[ERROR] Gagal mengimpor modul Sandbox dari paket 'blaxel'.")
        sys.exit(1)
...
async def _async_spawn(token, worker_name, full_command, cpu, memory_mb):
    # Mengatur API Key environment khusus per eksekusi
    os.environ["BLAXEL_API_KEY"] = token
    
    # Inisialisasi Sandbox lewat class Blaxel
    sb = Sandbox(
        name=worker_name,
        resources={
            "cpu": cpu,
    ...
    # Jika SDK membutuhkan pemanggilan method create / start secara async
    if hasattr(sb, "create"):
        res = sb.create()
        if asyncio.iscoroutine(res):
            await res
    elif hasattr(sb, "start"):
        res = sb.start()
        if asyncio.iscoroutine(res):
            await res
...
def spawn_single_sandbox(cfg, account, slot_index):
    ...
    try:
        # Jalankan coroutine asynchronous
        sbx = asyncio.run(_async_spawn(
            token=token,
            worker_name=worker_name,
            full_command=full_command,
            cpu=cfg.get("cpu_count", 2),
            memory_mb=cfg.get("memory_mb", 4096)
        ))

        sandbox_id = getattr(sbx, "id", getattr(sbx, "name", worker_name))
        now = datetime.now()
...
def deploy_all(cfg, accounts):
    ...
    print(f"⚡ Mode Mining           : SRBMiner-MULTI SDK ({cfg.get('algorithm', 'minotaurx')})")
    print("=======================================================\n")
...
def main():
    ...
    print(f"""
=============================================================
  ⚡ BLAXEL SRBMINER V3.6.5 AUTO-DEPLOYER (SANDBOX CLASS)
=============================================================
""")
