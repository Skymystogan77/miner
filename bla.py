...
# Cek & Import Blaxel SDK
try:
    from blaxel import BlaxelClient
except ImportError:
    print("[ERROR] Blaxel SDK belum terinstall! Silakan run: pip install blaxel requests")
    sys.exit(1)

# Default Fallback Config
DEFAULT_CONFIG = {
    "mode": "srbminer",
    "slots_per_key": 10,              # Max container per akun/workspace (Free Tier = 10)
    "cpu_count": 2,                  # Default Blaxel 2 vCPU
    "memory_mb": 4096,               # Default Blaxel 4 GB RAM
    "duration_minutes": 22,          # Auto-rerun interval dalam menit
    "auto_respawn": True,
    "algorithm": "minotaurx",
    ...
    "password": "c=DGB",
    "keys_file": "list.txt",          # Cukup isi Personal API Key per baris
    "concurrency_limit": 5,
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            print(f"[!] Gagal membaca {CONFIG_FILE}: {e}. Menggunakan default config.")
...
def parse_keys_file(filepath):
    ...
    with open(filepath, "r", encoding="utf-8-sig") as f:
        for line_no, raw_line in enumerate(f, 1):
            ...
            # Mendukung format token langsung atau token yang dipisah tanda pipe '|'
...
def spawn_single_sandbox(cfg, account, slot_index):
    ...
    try:
        # Menggunakan Personal API Key tanpa perlu spesifikasi workspace
        client = BlaxelClient(api_key=token)
        ...
        sandbox_id = getattr(sbx, 'id', worker_name)
        now = datetime.now()
...
def deploy_all(cfg, accounts):
    ...
    print(f"⚡ Mode Mining           : SRBMiner-MULTI ({cfg.get('algorithm', 'minotaurx')})")
    print(f"⚡ Resource per Sandbox : {cfg.get('cpu_count', 2)} vCPU | {cfg.get('memory_mb', 4096)} MB RAM")
    print("=======================================================\n")
...
def main():
    ...
    print(f"""
=============================================================
  ⚡ BLAXEL SRBMINER V3.6.5 AUTO-DEPLOYER (PERSONAL KEY)
=============================================================
""")
